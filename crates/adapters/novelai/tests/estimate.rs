...
use atelier_generation::{
    AnlasEstimateStatus, CharacterReference, CharacterReferenceType, GenerateImageRequest,
    GenerationPlanContext, ImageModel, ImageSize, Img2ImgRequest, VibeReference,
    VibeTransferConfig,
};
...
#[test]
fn flags_requests_above_the_per_image_limit() {
    let request = GenerateImageRequest {
        size: ImageSize {
            width: 1792,
            height: 1664,
    ...
    assert_eq!(estimate.per_image_cost, 143);
    assert_eq!(estimate.status, AnlasEstimateStatus::TooExpensive);
    ...
    let request = GenerateImageRequest {
        vibe_transfer: Some(VibeTransferConfig {
            references: vec![VibeReference {
                vibe_data_cache: "cached".to_owned(),
                strength: 0.6,
            }],
            strength: 0.6,
        }),
